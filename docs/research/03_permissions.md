...
Built with exploit-mitigation flags (stack-protector-strong, FORTIFY, PIE, full RELRO); libcap and libseccomp are mandatory whenever the helper is built, so it is never shipped unconfined. `-Dhelper=enabled` hard-fails if either is missing; the default `-Dhelper=auto` skips the helper and builds the library alone. **Recommended packaging:** install `root:<group>` mode `0750` — a world-executable file-capability binary on a multi-user host is a real consideration, and is handled on the consumer (RustDesk) side rather than inside libdrmtap.
