...
> **Update (libdrmtap)** — This snapshot's hypothesis held: the gap was real, and `libdrmtap` now fills it as the embeddable, MIT-licensed C library described here. It adopts the privileged-helper pattern first observed in gpu-screen-recorder (a small `drmtap-helper` carrying `CAP_SYS_ADMIN` via file capabilities, talking to the unprivileged library over a `socketpair`), and goes further with a zero-copy DMA-BUF + `SCM_RIGHTS` capture path (V3) and a GPU-universal EGL/GLES2 detiling backend. Verified here on Intel i915 (Meteor Lake-P), Nvidia (incl. Tegra/Jetson, aarch64), virtio-gpu and AMD `amdgpu` (RX560, gfx8); outside testers confirmed AMD RX Vega 64 (gfx9) and Intel Raptor Lake, one host each. HDR scanouts are tone-mapped to SDR ([#16](https://github.com/fxd0h/libdrmtap/issues/16), done) when the connector reports PQ (ST 2084): PQ decode, BT.2020 → BT.709 gamut and a highlight-preserving curve, for `AR30`/`XR30` and 16-bit `XR48`/`AR48` in the CPU and EGL paths. A plain SDR 10-bit scanout carries the same fourcc but no PQ metadata, and is simply reduced to 8-bit rather than tone-mapped (`P010` overlay YUV and HLG excepted).
