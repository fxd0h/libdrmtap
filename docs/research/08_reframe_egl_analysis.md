...
| **Role in libdrmtap** | Primary detile path; verified on Intel i915 (Meteor Lake-P here, Raptor Lake by a tester), Nvidia (incl. Tegra/Jetson), AMD amdgpu (RX560 here, RX Vega 64 by a tester), virtio-gpu | Fallback when EGL is unavailable; the AMD CPU-deswizzle path is not exercised on hardware (AMD was verified via the EGL path) |
