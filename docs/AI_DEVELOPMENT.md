...
- 🖥️ **Multi-GPU capture** — verified here on Intel i915 (Meteor Lake-P, multi-display 4K), Nvidia `nvidia-drm` including Tegra/Jetson (Orin Nano, aarch64), virtio-gpu, and AMD `amdgpu` (RX560, Polaris/gfx8). Outside testers have confirmed AMD RX Vega 64 (gfx9) and Intel Raptor Lake, one host each. A generic linear fallback covers other drivers.
...
This project is living proof of that thesis. The entire research corpus (9 documents, ~1,900 lines of technical analysis) was produced by one human + AI agents, and the shipped library grew directly out of it.
