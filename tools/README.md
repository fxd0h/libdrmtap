...
| Script | What it does |
| --- | --- |
| `set-version.sh X.Y.Z` | Stamps the 6 code sites (C header, csrc header, meson, the `libdrmtap-sys` crate, the wrapper crate's own version, and the wrapper's dependency on `libdrmtap-sys`). Leaves the wrapper crate's own separate version line and prose in docs untouched. |
...
`docs/research/` corpus. Those are intentionally not
