...
pub(super) async fn check_init() -> ResultType<()> {
    ...
    if !is_x11() {
        if CAP_DISPLAY_INFO.read().unwrap().is_empty() {
            ...
            let drm_mouse_rect = {
                let mut _drm_rect: Option<(i32, i32, i32, i32)> = None;
            ...
            if lock.is_empty() {
                ...
                // Check if displays are from DRM backend (libdrmtap) —
                // DRM displays already have correct positions and sizes,
                // skip PipeWire fill_displays() which requires RDP session info.
                #[cfg(feature = "drm")]
                ...
                if !is_drm {
                    ...
                    log::info!("DRM displays detected via libdrmtap — skipping PipeWire fill");
                    // Collect DRM display dimensions for uinput mouse resolution
                    // (stored for await outside the non-Send Display block)
                    if crate::input_service::wayland_use_uinput() {
                        ...
                        if maxx > 0 && maxy > 0 {
                            _drm_rect = Some((minx, maxx, miny, maxy));
                        }
                    }
                }
                *PIPEWIRE_INITIALIZED.write().unwrap() = true;
                ...
                _drm_rect
            }; // scope: lock + Display dropped here
            // Apply DRM mouse resolution outside the non-Send Display block
            if let Some((minx, maxx, miny, maxy)) = drm_mouse_rect {
                log::info!("DRM uinput resolution: ({}, {}), ({}, {})", minx, maxx, miny, maxy);
